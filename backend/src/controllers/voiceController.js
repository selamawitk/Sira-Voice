...
export const generateCV = asyncHandler(async (req, res) => {
  const { skill, experienceYears, location, availability, name } = req.body;
  ...
  const cvText = await generateCVFromProfile(profile);

  if (!cvText) {
    const userName = name || req.user?.fullName || 'Worker';
    const lines = [
      `=== CV - ${userName} ===`,
      '',
      `Name: ${userName}`,
      ...
      '',
      '--- Generated by Sira-Voice ---',
    ];
    return res.json({ success: true, cvText: lines.join('\n') });
  }

  res.json({ success: true, cvText });
});

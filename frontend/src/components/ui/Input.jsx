...
const Input = ({ icon: Icon, className = '', ...props }) => {
  return (
    <div className="relative group mb-3">
      ...
      <input
        {...props}
        className={`w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-11 pr-4 text-white text-sm placeholder:text-white/30 focus:border-[#2BB8B8]/40 focus:bg-white/[0.08] outline-none transition-all duration-300 relative z-0 ${className}`}
      />

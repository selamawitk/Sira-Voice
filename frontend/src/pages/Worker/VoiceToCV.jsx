...
import { Keyboard, Send, Mic, Square, CheckCircle2, Pencil, MapPin, Download, ArrowLeft, Sparkles, Loader2 } from 'lucide-react';
...
const VoiceToCV = () => {
  ...
  const processTranscript = async (text, session) => {
    ...
    try {
      ...
      console.error(err);
      const msg = err?.response?.data?.message || 'Failed to process CV from voice';
  ...
  useEffect(() => {
    if (transcript && !isListening && !isVoiceProcessing && !preview && !processingTranscript && voiceSessionRef.current > 0) {
      processTranscript(transcript, voiceSessionRef.current);
    }
  }, [transcript, isListening, isVoiceProcessing, processingTranscript]);
  ...
  const onStart = async () => {
    ...
    await startListening(null, { action: 'profile', raw: true });
  };
  ...
  const handleGenerateCV = async () => {
    if (!preview || generatingCV) return;
    setGeneratingCV(true);
    setCvGenerated(false);
    try {
      const res = await api.post('/voice/generate-cv', {
        skill: editableProfile.skill,
        experienceYears: editableProfile.experienceYears,
        location: editableProfile.location,
        name: auth?.user?.fullName || 'Worker',
      });
      const text = res.data?.cvText || '';
      if (text) {
        setCvText(text);
        setCvGenerated(true);
      } else {
        toast?.show?.('Failed to generate CV', 'error');
      }
    } catch (err) {
      console.error(err);
      toast?.show?.('CV generation failed', 'error');
    } finally {
  ...
  const handleDownloadCV = () => {
    ...
    a.href = url;
    a.download = `CV_${auth?.user?.fullName || 'Worker'}.txt`;
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden backdrop-blur-md flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="flex flex-col items-center gap-6 py-4">
              ...
              {!isListening && !isProcessing && !hasPreview && !showKeyboardFallback && (
                <div className="w-full bg-[#2BB8B8]/5 border border-[#2BB8B8]/20 rounded-2xl p-4 space-y-2">
                  <p className="text-[#2BB8B8] text-xs font-bold uppercase tracking-wider">What to say</p>
                  <ul className="text-white/60 text-xs space-y-1 list-disc list-inside">
                    <li>Your main <span className="text-white/80 font-semibold">skill</span> (e.g. plumber, carpenter, cleaner)</li>
                    <li>Your <span className="text-white/80 font-semibold">experience</span> in years</li>
                    <li>Your <span className="text-white/80 font-semibold">location</span> (neighborhood or city)</li>
                  </ul>
                  <p className="text-white/40 text-[10px] italic mt-1">Example: "I am a plumber with 3 years experience in Megenagna"</p>
                </div>
              )}
            </div>
            ...
            {__DEBUG__ && (
              <div className="mt-2 mb-2 bg-black/50 border border-yellow-500/30 rounded-2xl p-3 text-[10px] font-mono leading-relaxed">
                <p className="text-yellow-400 font-bold mb-1">DEBUG STATES</p>
                <p className="text-white/70">transcript: <span className="text-white">{transcript ? `"${transcript.slice(0, 40)}..."` : 'EMPTY'}</span></p>
                <p className="text-white/70">isListening: <span className="text-white">{String(isListening)}</span></p>
                <p className="text-white/70">preview: <span className="text-white">{preview ? 'SET' : 'null'}</span></p>
                <p className="text-white/70">processingTranscript: <span className="text-white">{String(processingTranscript)}</span></p>
                <p className="text-white/70">transcriptError: <span className="text-white">{transcriptError || '""'}</span></p>
                <p className="text-white/70">voiceSessionRef: <span className="text-white">{voiceSessionRef.current}</span></p>
                <p className="text-white/70">isVoiceProcessing: <span className="text-white">{String(isVoiceProcessing)}</span></p>
                <p className="text-yellow-400 font-bold mt-1">button show: <span className="text-white">{String(!!(transcript && !preview && !isListening && !processingTranscript && !transcriptError))}</span></p>
              </div>
            )}

            {processingTranscript && (
              <div className="mt-3 flex items-center gap-2 bg-[#2BB8B8]/10 border border-[#2BB8B8]/20 rounded-2xl px-4 py-3">
                <div className="inline-block w-4 h-4 border-2 border-[#2BB8B8]/20 border-t-[#2BB8B8] rounded-full animate-spin" />
                <p className="text-[#2BB8B8] text-xs font-medium">Analyzing your voice...</p>
              </div>
            )}

            {transcriptError && (
              <div className="mt-3 bg-red-500/10 border border-red-500/20 rounded-2xl px-4 py-3">
                <p className="text-red-400 text-xs">{transcriptError}</p>
              </div>
            )}

            {transcript && !preview && !isListening && !processingTranscript && !transcriptError && (
              <button
                onClick={() => processTranscript(transcript, voiceSessionRef.current)}
                className="mt-3 w-full bg-[#2BB8B8]/10 hover:bg-[#2BB8B8]/20 border border-[#2BB8B8]/30 text-[#2BB8B8] font-medium text-sm py-3 rounded-2xl transition-all flex items-center justify-center gap-2"
              >
                <Sparkles className="w-4 h-4" />
                Use this text to build CV
              </button>
            )}
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-white/10 pt-4 space-y-4">
                <button
                  ...
                  disabled={generatingCV || !preview}
                  className="w-full flex items-center justify-center gap-2 bg-purple-500/20 text-purple-400 border border-purple-500/30 font-black text-sm py-4 rounded-2xl hover:bg-purple-500/30 transition-all disabled:opacity-30"
                >
                  <Sparkles className="w-5 h-5" />
                  {copy.generateCV || 'Generate CV'}
                </button>

                {generatingCV && (
                  <div className="bg-[#1A2E35]/70 border border-purple-500/20 rounded-3xl p-6 text-center space-y-4 animate-fade-in">
                    ...
                    <p className="text-white font-black text-lg">{copy.generatingYourCV || 'Generating your CV...'}</p>
                    <div className="w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-purple-500 to-[#2BB8B8] rounded-full w-3/4 animate-pulse" style={{ animationDuration: '1.5s' }} />
                    </div>
                    <p className="text-white/40 text-xs">Building your professional profile document...</p>
                  </div>
                )}

                {cvGenerated && (
                  <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-5 space-y-4 animate-fade-in">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-[#2BB8B8]">Generated CV</p>
                    <pre className="bg-[#1A2E35]/70 border border-white/10 rounded-2xl p-4 text-white/80 text-sm font-mono whitespace-pre-wrap leading-relaxed">
                      {cvText}
                    </pre>
                    <div className="flex gap-3">
                      <button
                        onClick={handleDownloadCV}
                        className="flex-1 flex items-center justify-center gap-2 bg-[#2BB8B8] text-slate-950 font-black text-sm py-3 rounded-xl hover:brightness-110 transition-all"
                      >
                        <Download className="w-4 h-4" />
                        {copy.downloadCV || 'Download CV'}
                      </button>
                      <button
                        onClick={() => navigate(-1)}
                        className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 text-white font-black text-sm py-3 px-6 rounded-xl hover:bg-white/10 transition-all"
                      >
